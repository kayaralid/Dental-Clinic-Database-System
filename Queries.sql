...
select "Patient_id"
from "Patient" p
where "Patient_id" = (select "Patient_id" from "Patient" where "Patient_id" =
'39311235566');
...
from "Patient" p full outer join "Treated by" tb ON p."Patient_id" =
tb."Patient_ID" ;
...
/* Add an ‘Salary’ column to the existing Dentist table. Insert new dentists
to the Dentist table. Update the Dentist table by adding salaries to
existing dentists. */
...
select sum(d."Salary") as "Sum of the dentist's salaries whose salary is higher
than 2500"
from "Dentist" d
where d."Salary" > 2500;
...
/*Find the dentist’s name whose average salary is higher than 3000 and group them
by dentist name and salary. Order salary by ascending order. */
...
/* Find the union of the patient’s id’s whose dentist id is ‘31233503998’ and whose
disease id is ‘23181’ */
...
/* Find the dentist’s id’s who is work for ‘Hemdale Dental Center’ but not for
‘Sakada Dental’ */
(select "Dentist_ID" from "Works at" wa where "Dentalclinic_name"
= 'Hemdale Dental Center')
 except
(select "Dentist_ID" from "Works at" wa2 where "Dentalclinic_name" = 'Sakada
Dental');
...
/* Find the patient’s phone number whose exist in the inner query which shows the
patient’s surname’s end with the letter ‘g’. */
...
/* Find the dentist’s surnames whose not exist in the inner query which shows the
dentist’s room number is ‘130’ or ’140’. */
select "Dentist_Surname"
from "Dentist" d
where not exists (select *
from "Has Proficiency in" hpi
where d."Dentist_RoomNumber" = '130' or d."Dentist_RoomNumber"
='140');

/* Find the dentist’s room number and the maximum salaries the room have and
group by dentist’s room number. */
select d."Dentist_RoomNumber" , max("Salary") as "Highest salary in the belonging
room"
from "Dentist" d
where d."Dentist_RoomNumber" = '130' or d."Dentist_RoomNumber" = '120' or
"Dentist_RoomNumber" = '140'
group by d."Dentist_RoomNumber";

/* Find the dentist’s room number and the minimum salaries the room have.
Group by dentist’s room number in descending order. */
select d."Dentist_RoomNumber", min("Salary") as "Minimum salary in the belonging
room"
from "Dentist" d
where d."Dentist_RoomNumber" = '120' or "Dentist_RoomNumber" = '140' or
"Dentist_RoomNumber" = '130'
group by d."Dentist_RoomNumber"
...
/* Find the treatment names whose id’s last digit is not in the query which
shows the treatment id’s last digit is not 4. */
